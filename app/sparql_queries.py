...
def q_concurrent_contributors_monthly(g: Graph):
    ...
    for user, repo, ts in rows:
        # ts is an rdflib Literal dateTime; normalize to YYYY-MM
        try:
            # Try rdflib toPython for xsd:dateTime
            py_dt = ts.toPython()
            ...
            continue
        buckets[str(user)][ym].add(str(repo))
...
def main():
  ...
  if args.cmd == 'merges':
    ...
    for user, month, count in results:
      print(f"{user} @ {month} -> {count} repos")
  elif args.cmd == 'security':
    rows = q_security_messages(g, args.branch)
    for branch, commit, msg in rows:
      print(f"{branch} :: {commit} :: {msg}")
